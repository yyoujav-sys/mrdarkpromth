...
use mr_darkpromth_services::{ToolRegistry, ToolRegistryConfig, UserService, UserRepository};
...
impl Server {
    ...
    pub async fn start(&self, config: ServerConfig) -> std::io::Result<()> {
        ...
        let user_service = Arc::new(UserService::new(user_repository, self.jwt_secret.clone()));
        ...
        HttpServer::new(move || {
            ...
            let auth_middleware = AuthMiddleware::new(user_service.clone(), jwt_secret.clone());
