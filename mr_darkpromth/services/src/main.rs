...
use mr_darkpromth_services::{
    ...
    jailbreak_models::{CreatePromptRequest, PromptSearchRequest, JailbreakPrompt},
...
async fn health_check(state: web::Data<AppState>) -> impl Responder {
    ...
    let response = HealthResponse {
        ...
        services: serde_json::json!({
            ...
            "cerebras_integration": "operational",
...
// Tier-based Jailbreak Execution Endpoint with Cerebras.ai Integration
async fn execute_jailbreak(
    ...
    // Detect user tier - use tier from request if provided, otherwise check database
    let user_tier = if let Some(tier_str) = &request.tier {
        // Use tier from request
        match tier_str.to_lowercase().as_str() {
            "ultra" => CoreUserTier::Ultra,
            "premium" => CoreUserTier::Premium,
            _ => CoreUserTier::Free,
        }
    } else {
        // Check database
...
async fn main() -> Result<()> {
    ...
    let cerebras_api_key = env::var("CEREBRAS_API_KEY")
    ...
    let app_state = web::Data::new(AppState {
        ...
        cerebras_client: cerebras_client.clone(),
